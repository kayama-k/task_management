...
- 基本フロー:
  ...
  3. (初回起動時のみ)`To Do` / `In Progress` / `Done` の3リストが自動生成された状態で表示される。
...
| 層 | 技術 | 選定理由 | 備考 |
|---|---|---|---|
| フロントエンド | React + Vite | コンポーネント単位でUIを組み立てやすく、カンバンボードのようなインタラクティブなUIと相性が良い定番構成のため。Viteは開発サーバーの起動・ホットリロードが高速。 | - |
| ドラッグ&ドロップ | `@dnd-kit` | 同種のライブラリである `react-beautiful-dnd` は開発が終了(非推奨)しているため、現在も活発にメンテナンスされている `@dnd-kit` を採用。 | カードの並び替え・リスト間移動に使用 |
| バックエンド | Node.js + Express | 最小構成でREST APIを組める軽量フレームワークで、個人利用規模のアプリに対して過不足がないため。 | `/api/lists`, `/api/cards` を提供 |
| データベース | SQLite(`node:sqlite`) | 個人利用でサーバー管理が不要な軽量DBとしてSQLiteを選定。中でも `better-sqlite3` 等の外部パッケージではなくNode.js組み込みの `node:sqlite` を採用したのは、ネイティブビルドの失敗リスク(Windows環境でのビルドツール依存)を避け、追加の依存パッケージなしで動かせるため。 | Node.js 22.5以上が必要 |

### 10.2 検討したが採用しなかった技術

| 候補 | 不採用の理由 |
|---|---|
| Electron(デスクトップアプリ化) | 一時採用したが、ブラウザで使うTrello本来の使用感に合わせるため最終的にWebアプリ方式に戻し、不採用とした(11.検討・変更の経緯を参照)。 |
| Unsplash API連携(背景画像) | APIキー取得の手間に対して個人利用での必要性が低いと判断し、ローカル画像アップロード方式を採用した。 |
| `better-sqlite3` | ネイティブモジュールのビルドが環境依存で失敗するリスクがあるため、Node.js組み込みの `node:sqlite` を優先した。 |
